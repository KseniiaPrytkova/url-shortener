...
Hardcoding constants (port number 5000) is bad, we need to put them in the general config (`app.listen (5000, () => console.log (` App has been started ... `))`); install the `config` package: (`$ npm i config`) [config package](https://www.npmjs.com/package/config).
```
$ npm install config
$ mkdir config
...
## TODO:
- fix/find bugs - what if pass the link with slash at the end;
